...
class model_contato extends CI_Model {
	...
	function listall($dtvs = false){
		if($dtvs){
			$this->db->where('contato_status',1);
		}
		...
		return $this->db->get('tbl_contato')->result();
	}
	...
	function insert($data){
		$this->db->insert('tbl_contato',$data);
		return $this->db->insert_id();
	}
	function update($data){
		$this->db->where('contato_id',$data['contato_id']);
		return $this->db->update('tbl_contato',$data);
	}
	function delete($data){
		$this->db->where('contato_id',$data['contato_id']);
		return $this->db->delete('tbl_contato',$data);
	}
	function active($data){
		$data['contato_status'] = 1;
		$this->db->where('contato_id',$data['contato_id']);
		return $this->db->update('tbl_contato',$data);
	}
	function deactive($data){
		$data['contato_status'] = 0;
		$this->db->where('contato_id',$data['contato_id']);
		return $this->db->update('tbl_contato',$data);
	}
