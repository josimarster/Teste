...
class model_acesso extends CI_Model {
	...
	function insert($data){
		$this->db->insert('tbl_acesso',$data);
		return $this->db->insert_id();
	}
	function update($data){
		$this->db->where('acesso_id',$data['acesso_id']);
		return $this->db->update('tbl_acesso',$data);
	}
	function delete($data){
		$this->db->where('acesso_id',$data['acesso_id']);
		return $this->db->delete('tbl_acesso',$data);
	}
	function active($data){
		$data['acesso_status'] = 1;
		$this->db->where('acesso_id',$data['acesso_id']);
		return $this->db->update('tbl_acesso',$data);
	}
	function deactive($data){
		$data['acesso_status'] = 0;
		$this->db->where('acesso_id',$data['acesso_id']);
		return $this->db->update('tbl_acesso',$data);
	}
