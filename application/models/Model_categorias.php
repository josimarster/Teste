...
class Model_categorias extends CI_Model {
	...
	function listall($dtvs = false){
		if($dtvs){
			$this->db->where('categorias_status',1);
		}
	...
	function insert($data){
		$this->db->insert('tbl_categorias',$data);
		return $this->db->insert_id();
	}
	function update($data){
		$this->db->where('categorias_id',$data['categorias_id']);
		return $this->db->update('tbl_categorias',$data);
	}
	function delete($data){
		$this->db->where('categorias_id',$data['categorias_id']);
		return $this->db->delete('tbl_categorias',$data);
	}
	function active($data){
		$data['categorias_status'] = 1;
		$this->db->where('categorias_id',$data['categorias_id']);
		return $this->db->update('tbl_categorias',$data);
	}
	function deactive($data){
		$data['categorias_status'] = 0;
		$this->db->where('categorias_id',$data['categorias_id']);
		return $this->db->update('tbl_categorias',$data);
	}
